...
-   If the user needs to extend an agent's capabilities by integrating Model
    ...
    - **CRITICAL**: If wrapping a Google Antigravity Agent *inside* an MCP Server, ensure that no interactive console utilities (like `input()`) or standard output prints (`print()`) are used. These will immediately corrupt the JSON-RPC stdio protocol. Always redirect prints to standard error (`sys.stderr`).
