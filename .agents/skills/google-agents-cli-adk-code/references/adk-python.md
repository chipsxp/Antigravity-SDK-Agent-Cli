...
## 13. Executing Agents Synchronously (API & MCP Wrappers)

When wrapping an ADK agent inside a synchronous environment (like a FastMCP Server or a standard REST API), do not call `.run()` directly on the agent or App (this will raise an AttributeError). 

Instead, use `InMemoryRunner.run_debug()` as it abstracts away the complex `types.Content` parsing and session boilerplate required by the standard `run_async()` async generators.

### Example: Wrapping ADK in a FastMCP Server
...
# Assuming you have an ADK app configured
app = App(name="my_app", root_agent=root_agent)
mcp = FastMCP("My_MCP_Server")

@mcp.tool()
async def query_agent(query: str) -> str:
    # CRITICAL: Always redirect prints to sys.stderr inside MCP to prevent JSON-RPC corruption
    print(f"Received query: {query}", file=sys.stderr)
    
    runner = InMemoryRunner(app=app)
    response_text = ""
    
    # run_debug abstracts session creation and Content objects
    events = await runner.run_debug(query, quiet=True)
    
    for event in events:
        if getattr(event, 'content', None):
            # Extract clean text to strip out binary thought_signatures from reasoning models
